<template>
  <div>
    <div v-if="isSuccess">
      <!-- <div style="display: block;position: fixed;right: 10px;padding: 8px;top:100px">
      ...
      <div class="doc-container">
        <div class="border-1" style="min-height:1260px;">
          ...
          <div style="height:200px">
            <div @click="openSignaturePad" v-if="img" class="signature-1">
              <img :src="imgUrl(img)" width="280px" height="150px" />
            </div>
            <div v-else @click="openSignaturePad" class="signature-2">Sign Here</div>
          </div>
      ...
      <div class="save-docu-btn">
        <button @click="saveDocuSign()" class="btn btn-success" type="button">
...
export default {
  ...
  methods: {
    getDocusignData() {
      ...
      api
        .GetDocusignDataByToken(obj)
        .then(res => {
          if (res.data.IsSuccess) {
            this.img = res.data.Data.DocusignForm
              ? res.data.Data.DocusignForm.customer_signature
              : '';
            this.signImgForUpdate = res.data.Data.DocusignForm
              ? res.data.Data.DocusignForm.customer_signature
              : '';
            this.docData.id = res.data.Data.id;
            this.docData.pricefspaidforshipping =
              res.data.Data.pricefspaidforshipping;
    ...
    saveDocuSign() {
      if (this.signImgForUpdateBase64 != '') {
        ...
        api
          .CreateDocusignForm(obj)
          .then(res => {
            if (res.data.IsSuccess) {
              this.img = res.data.Data.customer_signature;
              this.signImgForUpdate = res.data.Data.customer_signature;
              this.$toast.open({
